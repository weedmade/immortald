...
var require_umd = __commonJS((exports, module) => {
  var Long = function(exports2) {
    ...
    var _default = exports2.default = Long2;
...
var require_channelz = __commonJS((exports) => {
  var __dirname = "/home/runner/work/openpond-sdk/openpond-sdk/node_modules/@grpc/grpc-js/build/src";
  Object.defineProperty(exports, "__esModule", { value: true });
...
var require_internal_channel = __commonJS((exports) => {
  ...
  class InternalChannel {
    constructor(target, credentials, options) {
      ...
      this.subchannelPool = (0, subchannel_pool_1.getSubchannelPool)(((_a = options["grpc.use_local_subchannel_pool"]) !== null && _a !== undefined ? _a : 0) === 0);
      this.retryBufferTracker = new retrying_call_1.MessageBufferTracker((_b = options["grpc.retry_buffer_size"]) !== null && _b !== undefined ? _b : DEFAULT_RETRY_BUFFER_SIZE_BYTES, (_c = options["grpc.per_rpc_retry_buffer_size"]) !== null && _c !== undefined ? _c : DEFAULT_PER_RPC_RETRY_BUFFER_SIZE_BYTES);
      this.keepaliveTime = (_d = options["grpc.keepalive_time_ms"]) !== null && _d !== undefined ? _d : -1;
      this.idleTimeoutMs = Math.max((_e = options["grpc.client_idle_timeout_ms"]) !== null && _e !== undefined ? _e : DEFAULT_IDLE_TIMEOUT_MS, MIN_IDLE_TIMEOUT_MS);
...
class Logger {
  ...
  static debug(context, message, meta) {
...
function resolvePackagePath(relativePath) {
  ...
  while (currentDir !== path.parse(currentDir).root) {
    const nodeModulesPath = path.join(currentDir, "node_modules", "@duckai", "sdk", relativePath);
    ...
    try {
      if (__require("fs").existsSync(nodeModulesPath)) {
  ...
  return path.join(__dirname2, "..", "..", relativePath);
}
...
class P2PClient {
  ...
  constructor(options) {
    ...
    this.protoPath = options.protoPath || resolvePackagePath("dist/proto/p2p.proto");
    this.binaryPath = options.binaryPath || resolvePackagePath("dist/node/p2p-node.js");
  ...
  async connect(nodeOptions) {
    try {
      await this.startNode(nodeOptions);
      await new Promise((resolve) => setTimeout(resolve, 2000));
      const packageDefinition = import_proto_loader.loadSync(this.protoPath, {
        keepCase: true,
        longs: String,
        enums: String,
        defaults: true,
      ...
      const { loadPackageDefinition } = await Promise.resolve().then(() => __toESM(require_src3(), 1));
      const proto = loadPackageDefinition(packageDefinition);
      let retries = 3;
      while (retries > 0) {
        try {
          this.client = new proto.p2p.P2PNode(this.options.address, import_grpc_js.credentials.createInsecure());
          ...
          this.stream.on("data", (event) => {
            Logger.info("p2p", "Received event from gRPC stream", {
              eventType: event.ready ? "ready" : event.peerConnected ? "peerConnected" : event.error ? "error" : event.message ? "message" : "unknown",
            ...
            if (event.message && this.messageHandler) {
              ...
              Logger.debug("p2p", "Received non-message event", {
                eventType: event.ready ? "ready" : event.peerConnected ? "peerConnected" : event.error ? "error" : "unknown",
                hasHandler: !!this.messageHandler
              });
            }
          });
  ...
  async findNodeExecutable() {
    try {
      ...
      for (const nodePath of possiblePaths) {
        try {
          if (nodePath) {
            __require("fs").accessSync(nodePath, __require("fs").constants.X_OK);
  ...
  async startNode(options) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        __require("fs").accessSync(nodeExecutable, __require("fs").constants.X_OK);
  ...
  async listAgents() {
    ...
    try {
      return new Promise((resolve, reject) => {
        this.client.ListAgents({}, (error, response) => {
          if (error) {
